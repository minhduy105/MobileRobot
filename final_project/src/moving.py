...
class Scheduler():
	...
	def personCallback(self, word):
		...
		waypoints.pop(0)
		print (self.person[str(word.data).upper()])
		print (waypoints)
	...
	def goTo(self,wayspoints):
		self.get_to_location = False
		self.in_motion = True        
				
		self.goal.target_pose.pose.position.x = self.waypoints[0]
		self.goal.target_pose.pose.position.y = self.waypoints[1]
		self.goal.target_pose.pose.position.z = 0
		
		self.goal.target_pose.pose.orientation.x = self.waypoints[2]
		self.goal.target_pose.pose.orientation.y = self.waypoints[3]
		self.goal.target_pose.pose.orientation.z = self.waypoints[4]
		self.goal.target_pose.pose.orientation.w = self.waypoints[5]
